...
if ($admin>=$edit_lvl || acl(XAT_CAN_EDIT) || acl(XTOTP_DISABLE_OTHERS)) { $canedit = 1; } else { $canedit = 0; }
	...
	echo ("</center></td></Tr>");
        ...
        if (!$edit) {
			...
			if ($no_totp_check == 1)
                ...
                if (($admin>0 || acl(XAT_CAN_EDIT))) echo("<TR><TD><font size=-1><b>Email</b></td><td><font size=-1> " . $user->email . "</font><FONT COLOR=#" . $cTheme->main_textlight . " size=-1> (Hidden from public)</td></tr>"); // Only show for self or admin
		if ($admin>0)
			{
...
if ((TOTP_ALLOW_ALL == 1) || ($admin > 0) || (isoper($user->user_id)))
	if (TOTP_ON)
		{
//if (SHOW_TOTP_PUBLIC || ($admin>0) || (isoper($user_id)))
      		//if (has_totp( $id))
	...
	if ($admin>0) {
		...
		if ($edit) {
			...
			else echo "<tr><td><b>User's Max Logins 4</b></td><td>" . ($user->maxlogins+0) . "&nbsp;</td></tr>\n";
		}
