...
if ($cookie!="") {
	...
	$q = "INSERT INTO users (user_name, password, flags, email, last_updated, last_updated_by, language_id, question_id, verificationdata, post_forms, created_ts, signup_ts, signup_ip, maxlogins)";
	$q .= sprintf(" VALUES ('%s', '%s', %d, '%s', %s, '%s', %d, %d, '%s', %s, %s, %s, '%s', %d)", $user->user_name, $crypt, 4, $user->email, "now()::abstime::int4", "Web Page New User", $user->language, $user->question_id, $verificationdata, "(now()::abstime::int4+432000)", "now()::abstime::int4", "now()::abstime::int4", cl_ip(), DEFAULT_MAX_LOGINS);
